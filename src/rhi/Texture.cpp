...
namespace DigitalTwin
{
    ...
    Result Texture::Create1D( uint32_t width, VkFormat format, TextureUsage usage )
    {
        TextureDesc desc;
        desc.width  = width;
        desc.height = 1;
    ...
    Result Texture::Create2D( uint32_t width, uint32_t height, VkFormat format, TextureUsage usage )
    {
        TextureDesc desc;
        desc.width  = width;
        desc.height = height;
    ...
    Result Texture::Create3D( uint32_t width, uint32_t height, uint32_t depth, VkFormat format, TextureUsage usage )
    {
        TextureDesc desc;
        desc.width  = width;
        desc.height = height;
    ...
    Result Texture::Create( const TextureDesc& desc )
    {
        ...
        {
            DT_CORE_CRITICAL( "Failed to create texture image!" );
            return Result::FAIL;
        }
        ...
        {
            DT_CORE_CRITICAL( "Failed to create texture image view!" );
            return Result::FAIL;
        }
    ...
    void Texture::TransitionLayout( Ref<CommandBuffer> cmd, VkImageLayout newLayout )
    {
        // TODO: Revisit
        ...
        barrier.subresourceRange.aspectMask     = GetAspectFlags( m_format );
        ...
        // Determine masks and stages based on layouts
        if( m_currentLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_GENERAL )
        {
        ...
        else if( m_currentLayout == VK_IMAGE_LAYOUT_GENERAL && newLayout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL )
        {
        ...
        // Additional transitions can be added here as needed

        cmd->PipelineBarrier( sourceStage, destinationStage, 0, {}, {}, { barrier } );
